...
module.exports = class x extends u {
  ...
  async ["execute"]() {
    ...
    try {
      ...
      this.client.consoleManager.log('[API 2] | [Create Module] Running the verification process', 7);
      ...
      for (const guildDataAPI of guildsData) {
        ...
        if (!existingGuildData) {
          ...
          if (!guildDataAPI.icon) {
            this.client.consoleManager.log(`[API 2] | [Create Module] An error has occurred while trying to add to the database the guild ${guildDataAPI.id}: No icon has been provided by the API.`, 5);
            return;
          }
          ...
          const newServer = joinedAtDate.isAfter(dayjs().subtract(14, 'days'));
          ...
          await GuildData.create({
            ...
            joinedAt: guildDataAPI.joinedAt ?? "1718834583891",
            ...
            this.client.consoleManager.log(`[API 2] | [Create Module] An error has occurred while trying to add to the database the guild ${guildDataAPI.id}: ${err}`, 5);
          });
          ...
          function generateServerHtml(guildDataAPI) {
            ...
            templateContent = templateContent
              ...
              .replace(/%bannerServer%/g, guildDataAPI.banner ? guildDataAPI.banner : `/footage/artworks/blackLotusDefault.webp`)
              ...
              .replace(/%descriptionServer%/g, guildDataAPI.description ? guildDataAPI.description : `${guildDataAPI.currentName} é um verdadeiro espaço acolhedor e vibrante para todos! Explore e compartilhe experiências únicas!`)
          ...
          this.client.consoleManager.log(`[API 2] | [Create Module] The guild ${guildDataAPI.id} has been successfully added to the database`, 0, 1);
        }
      }
      ('[API 2] | [Create Module] Stopping the verification process', 7)
    } catch (e) {
      this.client.errorManager.errorReport(this.client.configManager.get("lang").lang.unexpected_function_error.replace(/%function%/g, "dbCreateAPI2"), e);
    }
  }
